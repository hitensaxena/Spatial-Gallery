...
export function Environment() {
  ...
  const stars = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const r = 90 * Math.pow(Math.random(), 0.65)
  ...
  useFrame(({ clock }) => {
    ...
    if (scene.fog && 'density' in scene.fog) {
      const target = baseFogDensity + quiet * 0.038 + portal * 0.01
    ...
    if (starsRef.current) {
      ...
      starsRef.current.rotation.x = Math.sin(t * 0.03) * 0.05
    }
  })

  return (
    <>
      <points ref={starsRef} geometry={stars.geometry} material={stars.material} frustumCulled={false} renderOrder={-999} />
